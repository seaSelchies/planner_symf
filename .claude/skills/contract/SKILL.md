...
`/spec` established **what** a module does and wrote it to `docs/specs/<name>.md`. This step
decides **what shape** that module takes — its name, its ports, its signatures — and writes those
...
existing file can be modified. `Write` stays for the new declarations only.

**The only paths this skill may write are new files under `src/<Module>/`, plus a new
...
`docs/` to `/spec`, `config/` and `migrations/` to a human, and a file that already exists to nobody
at this step.
If a write is denied, that is the rule working, not an obstacle to route around.
...
1. **Read the specification and derive what the module needs.** Read the whole document, not the
   ...
   - classes in `Application/` and `Domain/` are `final`, except a Doctrine entity (invariant 16)
...
2. **Present the proposal in one message.** Everything the human needs in order to answer, with
   ...
   - every **domain class**: name, method signatures, and whether it is an entity (non-final) or a
     rule class (final)
   ...
   - the **full path each file will take**
...
- **Concrete classes** — handlers, rule classes, entities, controllers, adapters — where every
...
- `php -l` on every file created — a parse error in a declaration breaks the whole next step.
