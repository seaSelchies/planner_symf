...
7. Every schema change ships as a new migration file under `migrations/`, expressed through the DBAL schema API, and is applied only after a human has approved it. Raw SQL via `addSql()` is permitted only for a construct that API cannot express — a `CHECK` constraint, a partial or expression index, an enum type, a data backfill — and the migration names the construct and the reason in a comment directly above the call. A table with no entity of its own, such as a link table, is an ordinary table and is created through the API like any other.
