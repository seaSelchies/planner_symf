...
| C25 | **Partially covered.** The mechanism — the streak stops the instant a day is absent from both maps — is covered by `FodmapStreakCalculatorTest::testStreak` (`'C25 mechanism: ...'`). The ~112–118 day magnitude itself is an Infrastructure window-size detail (see the `LookbackWindow` docblock) with no Domain counterpart to assert a specific day count against. |
| C26 | **Not covered as originally stated.** The original discards the Supabase `error` field; this port deliberately does not reproduce that (invariant 37). Superseded by `T:fodmap-streak-fetch-error-handling` below. |
...
| C30 | **Not covered — the spec's expected result is no longer producible by this port.** The spec (C30) expects `"moderate"`, resolved in the original via the `recipe_ingredients.fodmap_tier` column fallback. That column is dropped (migration 026); C30's only realizable input under the single-source shape is "no catalogue link" (`catalogueTier = null`), and `FodmapTier::orUnknown(null)` returns `Unknown`, never `Moderate` — there is nothing left in the schema to recover `"moderate"` from. This is a recorded, deliberate divergence, not an oversight, and it **crosses `isBad()`'s boundary** (`Moderate` = unsafe → `Unknown` = safe) — a real difference in the resulting streak count for a day whose only tier evidence takes this shape, not merely a relabeling. `FodmapTierTest::testOrUnknown`'s `null → Unknown` entry (labeled for C31) exercises the same input C30 now maps to, but its output does not match what C30's spec row expects, so it does not cover C30. |
...
- **C30: was marked Covered, now marked Not covered.** No test was removed — the coverage claim
  ...
  producible by `FodmapTier::orUnknown()` at all; see the C30 row in the Cases table above for why, and for the
  finding that this crosses `isBad()`'s boundary.
